...
public class CasasMorosas extends javax.swing.JFrame {
    ...
    public CasasMorosas() {
        ...
        javax.swing.ImageIcon BuscarCasa = new javax.swing.ImageIcon(
                VistaVerdeIconos.getBucar().getImage()
                        .getScaledInstance(25, 25, java.awt.Image.SCALE_SMOOTH)
        );
        jButton2.setIcon(BuscarCasa);
    ...
    private void buscarPorMes() {
        ...
        jTable2.getColumnModel().getColumn(2).setHeaderValue("Teléfono");
        jTable2.getTableHeader().repaint();

        if (morosos.isEmpty()) {
            jLabel8.setText("Total de casas Morosas: 0");
            JOptionPane.showMessageDialog(
                    this,
                    "No hay casas morosas registradas para el mes de "
    ...
    private void buscarPorCasa() {
        ...
        if (idPropietario == -1) {
            JOptionPane.showMessageDialog(
                    this,
                    "La casa " + idCasa + " no tiene propietario asignado.\nNo se puede verificar morosidad.",
                    "Casa sin propietario",
                    JOptionPane.WARNING_MESSAGE
            );
            jLabel8.setText("Casa " + idCasa + " no tiene propietario.");
            return;
        }

        ArrayList<Integer> mesesTardios = MorososDB.getMesesTardiosPorCasa(idCasa);

        DefaultTableModel modelo = (DefaultTableModel) jTable2.getModel();
        modelo.setRowCount(0);

        jTable2.getColumnModel().getColumn(2).setHeaderValue("Registro de Pagos");
        jTable2.getTableHeader().repaint();

        if (mesesTardios.isEmpty()) {
            JOptionPane.showMessageDialog(
                    this,
                    "La casa " + idCasa + " NO tiene pagos tardíos registrados.",
                    "Casa al día",
                    JOptionPane.INFORMATION_MESSAGE
            );
            jLabel8.setText("Casa " + idCasa + " está al día.");
        } else {
            ...
            for (int m : mesesTardios) {
                modelo.addRow(new Object[]{
                    idCasa,
                    logic.PagoDB.obtenerNombrePropietarioPorCasa(idCasa),
                    "Pago tardío en " + nombreMeses[m]
                });
            }

            jLabel8.setText("Casa " + idCasa + " tiene " + mesesTardios.size() + " pagos tardíos.");
        }
    }
